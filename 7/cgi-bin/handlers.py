...
print(f'Длина файла "names.txt": {names_file_size}<br>')
print(f'Длина файла "values.txt": {values_file_size}<br>')


with open('values.txt', 'r') as values_file:
    print(f'Содержимое файла "values.txt":<br>')
    print(values_file.read().replace('\n', '<br>'))
...
with open('binary_data.dat', 'rb') as binary_file:
    ...
    print(f'Значение 20-го байта: {data[20]}<br>')
    try:
        binary_file.seek(-4, 2)
        print(f'Значение трех байтов: {binary_file.read(3)}<br>')
    except OSError:
        print('Недопустимые значения для "offset" и "whence"<br>')

    digit = str(data[-1])[-1]
    try:
        assert digit == 3
        print(f'Последняя цифра {digit} в последнем числе последовательности совпадает с целевой<br>')
    except AssertionError:
        print(f'Последняя цифра {digit} в последнем числе последовательности не совпадает с целевой<br>')
